...
<section style="position:relative;min-height:45vh;display:flex;align-items:center;padding-top:6rem;padding-bottom:3rem;overflow:hidden">
    ...
    <div style="position:absolute;inset:0;background:linear-gradient(180deg,transparent 40%,rgba(168,85,247,0.02) 100%);pointer-events:none"></div>
    <div style="position:relative;z-index:10;max-width:80rem;margin:0 auto;padding-left:1rem;padding-right:1rem;text-align:center;width:100%">
        ...
        <div style="display:flex;flex-wrap:wrap;justify-content:center;gap:1rem;animation:fadeInUp 0.6s ease 0.3s both">
            <div style="display:flex;align-items:center;gap:0.5rem;padding:0.5rem 1.25rem;background:rgba(168,85,247,0.04);border:1px solid rgba(168,85,247,0.1);border-radius:9999px;font-size:0.8125rem;color:rgba(234,234,234,0.5);transition:all 0.3s">
                <svg style="width:1rem;height:1rem;color:#A855F7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/></svg>
            ...
            <a href="#src-grid" style="display:inline-flex;align-items:center;gap:0.375rem;padding:0.5rem 1.25rem;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:9999px;font-size:0.8125rem;color:rgba(234,234,234,0.5);transition:all 0.3s" 
               onmouseover="this.style.borderColor='rgba(168,85,247,0.2)';this.style.color='#EAEAEA'" 
...
<section id="src-grid" style="padding-top:1rem;padding-bottom:6rem;position:relative;overflow:hidden">
    <div class="orb orb-brand" style="position:absolute;top:0;left:0;transform:translate(-50%,-50%);opacity:0.04;width:500px;height:500px;border-radius:50%;background:rgba(168,85,247,0.15);filter:blur(100px);pointer-events:none"></div>
    <div style="position:relative;z-index:10;max-width:80rem;margin:0 auto;padding-left:1rem;padding-right:1rem">
        <style>
            ...
            .src-meta-item:hover{background:rgba(168,85,247,0.04);border-color:rgba(168,85,247,0.1);color:rgba(234,234,234,0.6)}
            /* Pagination styling */
        ...
        <div class="src-grid">
            ...
            <div style="text-decoration:none;color:inherit;display:block;transition-delay:{{ $i * 0.06 }}s" class="reveal" onclick="var t=event.target; while(t){if(t.classList&&t.classList.contains('src-atc-btn'))return; t=t.parentNode} window.location.href='{{ route('forex.source-code-detail', $sc->slug) }}'">
                <div class="glass-card" style="height:100%;display:flex;flex-direction:column;overflow:hidden;cursor:pointer;position:relative;transition:all 0.4s cubic-bezier(0.4,0,0.2,1)"
                     onmouseover="this.style.transform='translateY(-6px)';this.style.boxShadow='0 20px 60px -12px rgba(0,0,0,0.5), 0 0 0 1px rgba(168,85,247,0.22)'"
                     onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='none'">
                    {{-- Top accent line --}}
                    <div style="position:absolute;top:0;left:1.5rem;right:1.5rem;height:2px;background:linear-gradient(90deg,#A855F7,rgba(168,85,247,0.2),transparent);border-radius:0 0 2px 2px;opacity:0.5"></div>
                    {{-- Glow orb --}}
                    <div style="position:absolute;top:-3rem;right:-3rem;width:10rem;height:10rem;background:radial-gradient(circle,rgba(168,85,247,0.05),transparent 70%);pointer-events:none;border-radius:50%;transition:opacity 0.5s;opacity:0;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0'"></div>
                    <div style="height:100%;display:flex;flex-direction:column;flex:1;padding:1.5rem;position:relative;z-index:2">
                        <!-- Icon / Image -->
                        <div style="width:3.5rem;height:3.5rem;border-radius:0.875rem;display:flex;align-items:center;justify-content:center;margin-bottom:1rem;transition:all 0.3s;background:linear-gradient(135deg, rgba(168,85,247,0.15), rgba(124,58,237,0.05));overflow:hidden;border:1px solid rgba(168,85,247,0.12)"
                             onmouseover="this.style.transform='scale(1.08)';this.style.background='linear-gradient(135deg, rgba(168,85,247,0.25), rgba(124,58,237,0.1))'"
                             onmouseout="this.style.transform='scale(1)';this.style.background='linear-gradient(135deg, rgba(168,85,247,0.15), rgba(124,58,237,0.05))'">
                            @if($sc->image)
                            ...
                            @else
                            <svg style="width:1.75rem;height:1.75rem;color:#A855F7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/></svg>
                            @endif
                        </div>

                        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:6px">
                            <h3 style="color:#EAEAEA;font-weight:600;font-size:1.125rem;margin:0;transition:color 0.3s" onmouseover="this.style.color='#A855F7'" onmouseout="this.style.color='#EAEAEA'">{{ $sc->name }}</h3>
                            @if($sc->language)
                            <span style="padding:3px 10px;background:rgba(168,85,247,0.1);border:1px solid rgba(168,85,247,0.15);border-radius:50px;font-size:10px;font-weight:600;color:#A855F7;letter-spacing:0.02em">{{ strtoupper($sc->language) }}</span>
                            @endif
                            @if($sc->category)
                        ...
                        <div style="display:flex;align-items:center;justify-content:space-between;padding-top:1rem;margin-top:auto;border-top:1px solid rgba(255,255,255,0.06)">
                            ...
                            <div style="display:flex;align-items:center;gap:0.5rem">
                                <button class="src-atc-btn" onclick="event.stopPropagation();addToCart({name:'{{ $sc->name }} Source Code', price:{{ $sc->price }}, id:'src-{{ $sc->slug }}'})" style="display:inline-flex;align-items:center;gap:0.375rem;padding:0.5rem 0.875rem;background:linear-gradient(135deg,#A855F7,#7C3AED);color:white;font-weight:600;font-size:0.75rem;border-radius:0.625rem;transition:all 0.3s;cursor:pointer;border:none;font-family:'DM Sans',sans-serif"
                                      onmouseover="this.style.transform='translateY(-1px)';this.style.boxShadow='0 4px 15px rgba(168,85,247,0.25)'"
                                ...
                                <a href="{{ route('forex.source-code-detail', $sc->slug) }}" style="display:inline-flex;align-items:center;gap:3px;padding:6px 10px;background:rgba(168,85,247,0.08);border:1px solid rgba(168,85,247,0.15);color:#A855F7;border-radius:8px;font-size:11px;font-weight:500;transition:all 0.3s;text-decoration:none"
                                   onmouseover="this.style.background='rgba(168,85,247,0.18)';this.style.borderColor='#A855F7'"
                                   onmouseout="this.style.background='rgba(168,85,247,0.08)';this.style.borderColor='rgba(168,85,247,0.15)'"
        ...
        <div style="text-align:center;padding:5rem 0;max-width:32rem;margin:0 auto">
            <div style="width:5.5rem;height:5.5rem;margin:0 auto 1.5rem;border-radius:1rem;background:rgba(168,85,247,0.04);border:1px solid rgba(168,85,247,0.08);display:flex;align-items:center;justify-content:center;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px)">
                <svg style="width:2.5rem;height:2.5rem;color:#A855F7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/></svg>
            </div>
            ...
            <div style="display:flex;flex-wrap:wrap;justify-content:center;gap:0.75rem">
                <a href="{{ route('forex.products') }}" style="display:inline-flex;align-items:center;gap:0.375rem;padding:0.625rem 1.25rem;background:linear-gradient(135deg,#A855F7,#7C3AED);color:white;font-weight:600;font-size:0.8125rem;border-radius:0.625rem;transition:all 0.3s;text-decoration:none"
                   onmouseover="this.style.boxShadow='0 8px 25px rgba(168,85,247,0.2)';this.style.transform='translateY(-2px)'"
